    ...
    */
    _buildNotification: function($this) {
      ...
      if ( data.settings.sticky ) {
        ...
        close.click( { callback: data.settings.callback, message_id: data.settings.message_id }, function(event) {
          notification.animate({width: 'hide'}, {
            duration: 'fast',
            complete: function() {
              notification.remove();
              typeof event.data.callback === 'function' && event.data.callback( event.data.message_id );
            }
          });
        });
    ...
    switch (message) {
      ...
      default:
        ...
        if ($('.jquery-notific8-container').size() === 0) {
          ...
          $body.append($('<div />').addClass('jquery-notific8-container').addClass('ntop').addClass('right'));
          $body.append($('<div />').addClass('jquery-notific8-container').addClass('ntop').addClass('left'));
          $body.append($('<div />').addClass('jquery-notific8-container').addClass('nbottom').addClass('right'));
          $body.append($('<div />').addClass('jquery-notific8-container').addClass('nbottom').addClass('left'));
        ...
        if ((!options.hasOwnProperty('horizontalEdge')) || ((options.horizontalEdge.toLowerCase() != 'ntop') && (options.horizontalEdge.toLowerCase() != 'nbottom'))) {
