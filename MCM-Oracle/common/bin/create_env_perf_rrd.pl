...
my $app_server  = Mx::Util->hostname( $app_servers[0] );
...
foreach my $env ( @ENVS ) {
    next if $SEEN{ $env };
    $SEEN{ $env } = 1;
    push @options, ( data_source => { name => "cpu_$env", type => 'GAUGE' } );
    push @options, ( data_source => { name => "mem_$env", type => 'GAUGE' } );
}

push @options, ( data_source => { name => 'cpu_OTHER', type => 'GAUGE' } );
push @options, ( data_source => { name => 'mem_OTHER', type => 'GAUGE' } );
