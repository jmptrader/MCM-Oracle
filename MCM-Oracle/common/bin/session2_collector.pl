...
while ( 1 ) {
    my %sessions = $db_audit->retrieve_session_counts();

    my %values = ();
    foreach my $mx_scripttype ( keys %sessions ) {
        next if ! $mx_scripttype or $mx_scripttype eq 'server process';

        my $mx_scripttype_ws = $mx_scripttype;
        $mx_scripttype_ws =~ s/ //g;

        foreach my $hostname ( keys %{$sessions{$mx_scripttype}} ) {
            my $count = $sessions{$mx_scripttype}->{$hostname};
            my $label = $hostname . $mx_scripttype_ws;
            $values{$label} = $count;
            $values{$mx_scripttype_ws} += $count;
        }
    }
