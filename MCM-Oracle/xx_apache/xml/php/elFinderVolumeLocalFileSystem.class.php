...
class elFinderVolumeLocalFileSystem extends elFinderVolumeDriver {
	...
	protected function _stat($path) {
		...
		$process = proc_open("readlink -f $path | xargs /opt/CollabNet_Subversion/bin/svnversion", $descriptorspec, $pipes, NULL, null,array("bypass_shell"=>false));
		
		$res = fgets($pipes[1]);
		
		$reverse = strrev( "$res" );

		if($reverse[1] == "M"){$stat['revision'] = "Modified";}
		else{
			$stat['revision'] = $res;
		}
		return $stat;
